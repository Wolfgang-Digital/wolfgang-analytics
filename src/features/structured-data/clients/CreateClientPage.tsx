...
import { Tabs, TabPanels, TabPanel, FormControl, FormLabel, FormHelperText, Button, Heading } from '@chakra-ui/core';
...
const organisationTypes = ['Organization', 'Local Business'].sort().map(type => ({ label: type, value: type }));
      ...
      <TabPanels outline={0}>
        <TabPanel height="calc(100vh - 300px)" display="flex" flexDirection="column" outline={0}>
          <Card maxWidth="720px" width="100%" margin="auto">
            <Heading size="md" mb={4}>Create Client</Heading>
            <FormControl isRequired>
              <FormLabel>Type</FormLabel>
              <Select value={type} options={organisationTypes} onChange={selected => setType(selected as SelectOption)} />
